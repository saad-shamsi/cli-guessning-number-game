...
async function askQuestion() {
    ...
    do {
        ...
        if (que.usr_num === randomNum) {
            console.log(chalk.blue `HURRAH!` + chalk.greenBright ` You Guessed A Right Number!!!`);
        }
        else if (que.usr_num > randomNum) {
            console.log(chalk.redBright(`Your guess is High! ` +
                chalk.blueBright `Try a Lower number this time `));
        }
        else if (que.usr_num < randomNum) {
            console.log(chalk.redBright(`Your guess is Low! ` + chalk.blueBright `Try a Higher number this time `));
        }
        ...
        var restart = await inquirer.prompt([
            {
                name: "usr_restart",
                type: "input",
                message: chalk.yellowBright("Do you want to play Again?" + chalk.blue `Press Y or N`),
            },
        ]);
    } while (restart.usr_restart === "Y" ||
